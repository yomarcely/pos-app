  ...
  return db.transaction(async (tx) => {
    ...
    for (const tenant of tenants) {
      ...
      if (insertedSellers.length > 0 && insertedEstablishments.length > 0) {
        ...
        await tx.insert(sellerEstablishments).values(sellerEstablishmentValues)
      }
    ...
    const tables = [
      ...
      'sellers', 'seller_establishments', 'customers', 'products', 'product_stocks', 'product_establishments', 'customer_establishments', 'establishments', 'registers', 'sales', 'sale_items',
