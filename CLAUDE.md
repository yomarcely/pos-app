...
| 2026-03-24 | Bugs B5-B11 | B5 : `geo.api.gouv.fr` ajouté dans CSP `connect-src`. B6 : remise cappée à 100% (%) / prix unitaire (€) dans `cartUtils.ts` + `updateDiscount`. B7 : `useEstablishments` appelle `useEstablishmentRegister().loadEstablishments()` après chaque mutation. B8 : `GET /api/establishments` accepte `?includeInactive=true` — page de gestion affiche tous les établissements. B9 : `sessionRestored` dans auth store + placeholder de chargement dans dashboard layout. B10 : suppression TVA bloquée (409) si produits l'utilisent. B11 : `useEstablishmentRegister.reset()` appelé lors du signOut — rechargement pour le prochain utilisateur — 384 tests / 66 fichiers — 0 échec | ✅ |

*Dernière mise à jour : 2026-03-24 — par Claude Code (session bugs B5-B11)*
