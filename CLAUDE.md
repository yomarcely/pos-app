...
| 2026-03-24 | Bugs B5-B11 | B5 : `geo.api.gouv.fr` ajouté dans CSP `connect-src`. B6 : remise cappée à 100% (%) / prix unitaire (€) dans `cartUtils.ts` + `updateDiscount`. B7 : `useEstablishments` appelle `useEstablishmentRegister().loadEstablishments()` après chaque mutation. B8 : `GET /api/establishments` accepte `?includeInactive=true` — page de gestion affiche tous les établissements. B9 : plugin `02.session-restore.client.ts` restore la session avant montage — `sessionRestored` dans auth store. B10 : suppression TVA bloquée (409) si produits l'utilisent. B11 : `useEstablishmentRegister.reset()` appelé lors du signOut — rechargement pour le prochain utilisateur — 384 tests / 66 fichiers — 0 échec | ✅ |
| 2026-03-24 | Fix page centrée après redirect login | Suppression du `v-if !sessionRestored` dans `layouts/dashboard.vue` — le plugin `02.session-restore.client.ts` garantit que le middleware redirige avant tout rendu du layout dashboard | ✅ |

**Note B9 — Architecture auth côté client** :
- `plugins/02.session-restore.client.ts` : appelle `restoreSession()` avant que l'app monte (lit le localStorage Supabase, pas de requête réseau). Garantit que `isAuthenticated` et `sessionRestored` sont connus avant le premier render côté client.
