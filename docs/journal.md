...
| 2026-07-03 | Bootstrap base staging | Découverte : chaîne de migrations non rejouable de zéro (22 tables/32 sans CREATE TABLE — schéma initial historique via db:push). Base staging Supabase amorcée : `db:push` (32 tables) + nouveau script `scripts/mark-migrations-applied.ts` (marque les 23 migrations du journal comme appliquées, garde-fous RUN_BOOTSTRAP=1/journal vierge/schéma présent) + `db:migrate` idempotent vérifié. Runbook staging corrigé (étape 2 bootstrap), risque ouvert ajouté à CLAUDE.md. Réutilisable pour l'amorçage prod | ✅ |
| 2026-07-03 | Staging Vercel — 3 incidents serverless | (1) Région iad1 → cdg1 (réglage Vercel). (2) Sockets DB morts au réveil d'instance gelée → `connection.ts` : idle_timeout 20s / max_lifetime 30min / connect_timeout 10s. (3) Transaction zombie : fonction tuée à maxDuration (10s) en pleine vente → `pg_advisory_xact_lock` jamais libéré, caisse paralysée — réglé par maxDuration 60s (nitro.vercel), `idle_in_transaction_session_timeout=30s` (option session + ALTER DATABASE staging). Diagnostic et procédures dans le runbook deploy-staging-vercel. 844 tests verts | ✅ |
| 2026-07-03 | Fix deadlock vente (audit dans transaction) | Cause racine des ventes qui « tournent en boucle » sur Vercel : `logSaleCreation` appelé DANS `db.transaction` mais écrivant via la connexion globale → en mode pooler (max 1 connexion) la transaction attend l'audit qui attend la connexion (auto-deadlock, tué à 30s par idle_in_transaction). Reproduit hors Vercel (build local + base staging : 31,7s→500), corrigé (`DbExecutor` optionnel sur `logAuditEvent`/`logSaleCreation`, `tx` passé par sales/create — audit atomique avec la vente), re-testé : vente en 1,7s, tickets 000001/000002 chaînés. Sweep des 12 fichiers à transaction : seul sales/create était touché. Le test roles.test.ts (db-getter qui lève) a attrapé une régression du contrat « l'audit ne bloque jamais » (défaut de param évalué avant le try) → résolution `db` déplacée dans le try. Données bench purgées du staging. 844 tests verts | ✅ |
| 2026-07-03 | Même deadlock : mouvements & inventaires | Suite du fix précédent, signalé par Yohan sur réception fournisseur/mouvement de stock. Sweep exhaustif (awk sur les 12 blocs `db.transaction` : tout `await` non-`tx`) → 3 sites fautifs : `createMovement` (utilisé par movements/create ET inventory-preparations/validate) et `createInventoryPreparation` (inventory-preparations/index) — helpers écrivant via `db` global depuis une transaction. Fix : `DbExecutor` centralisé dans `connection.ts`, param `executor` sur les 2 helpers, `tx` passé aux 3 sites. 4 tests de caractérisation mis à jour (arg `tx` attendu). Vérifié sur banc local→staging : réception 0,64s (stock 0→5), prépa inventaire 0,90s. 844 tests verts | ✅ |
